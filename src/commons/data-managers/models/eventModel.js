...
EventSchema.pre('save', function(next) {
  if (this.information) {
    if (this.information.startDate) {
      this.information.startDate = formatDateToYYYYMMDD(this.information.startDate);
    }
...
EventSchema.pre('updateOne', function(next) {
  ...
  if (update && update.information) {
    if (update.information.startDate) {
      update.information.startDate = formatDateToYYYYMMDD(update.information.startDate);
    }
    if (update.information.endDate) {
      update.information.endDate = formatDateToYYYYMMDD(update.information.endDate);
    }
  }
  next();
});
