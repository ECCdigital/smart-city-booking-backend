...
class NukiAccessProvider extends AccessProvider {
  ...
  validateAccessPoint(accessPoint, listedAccessPoint) {
    ...
    // A missing key - or an explicit `null`, which the open reads the same
    // way - means `auto`, and `auto` is the platform's own choice at open
    // time: neither is a promise about this device.
    if (openAction == null || openAction === NUKI_OPEN_ACTIONS.AUTO) {
      return;
    }
