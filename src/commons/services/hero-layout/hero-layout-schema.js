...
const BLOCK_PANELS = Object.freeze(["none", "translucent"]);
...
/**
 * The colour of a text or rich-text Block: one of the contract's named tokens
 * or a `#rrggbb`. A Background takes hex alone; this is the wider rule.
 *
 * @param {ObjectReader} reader - The Block being read.
 ...
 */
function readContentColor(reader, key, fallback) {
  ...
  if (!CONTENT_COLOR_TOKENS.includes(value) && !HEX_COLOR.test(value)) {
...
const BLOCK_FAMILIES = Object.freeze({
  text: (reader) => ({
    ...
    size: readEnum(reader, "size", TEXT_SIZES, "md"),
    color: readContentColor(reader, "color", "default"),
...
function readBlock(input, path, errors, collectors) {
  ...
  const block = {
    ...
    panel: readEnum(reader, "panel", BLOCK_PANELS, BLOCK_DEFAULTS.panel),
