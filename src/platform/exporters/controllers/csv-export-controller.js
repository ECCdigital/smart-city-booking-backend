...
class CsvExportController {
  ...
  static async _hasPermission(event, userId, tenant) {
    ...
    static async getEventBookings(request, response) {
        const { params: { tenant: tenantId, id: eventId }, user } = request;
        ...
        const eventTickets = bookables.filter(b => b.type === "ticket" && b.eventId == eventId);

        const bookings = await BookingManager.getBookings(tenantId);
        const eventBookings = bookings
            .filter(b => {
                const validIds = b.bookableIds.length > 0 ? b.bookableIds : b.bookableItems.map(item => item.bookableId);
