...
class BookableController {
  ...
  static async createBookable(request, response, next) {
    try {
      ...
      BookableController._dropAccessPointAmounts(bookable);
  ...
  /**
   * Drops `accessPointDetails.accessPointAmounts`, the distribution of a
   * bookable's `amount` over its locker systems that is gone: a booking gets
   * one compartment per booked unit at each of them, there is no number per
   * system. A client that still sends the field is not refused, the field is
   * simply not stored - `accessPointDetails` is a plain object to the
   * schema, so nothing else would drop it.
   *
   * @param {Bookable} bookable The bookable to be stored, changed in place
   */
  static _dropAccessPointAmounts(bookable) {
    if (bookable.accessPointDetails) {
      delete bookable.accessPointDetails.accessPointAmounts;
    }
  }
