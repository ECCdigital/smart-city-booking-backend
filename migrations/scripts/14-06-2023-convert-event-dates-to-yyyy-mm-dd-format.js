module.exports = {
    ...
    for (const event of events) {
      ...
      if (event.information) {
        ...
        if (updated) {
          ...
          console.log(
            `Updated event ${event.id}: startDate=${event.information.startDate}, endDate=${event.information.endDate}`,
          );
        }
      }
    }
  ...
  down: async function (mongoose) {
    console.log(
      "This migration cannot be reverted as the original date formats are not stored",
    );
  },
};
