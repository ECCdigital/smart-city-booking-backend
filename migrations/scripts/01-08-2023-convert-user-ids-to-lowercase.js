module.exports = {
  ...
  up: async function (mongoose) {
    ...
    const bookings = await Booking.find({
      assignedUserId: { $exists: true, $ne: "" },
    });

    for (const booking of bookings) {
      if (
        booking.assignedUserId &&
        booking.assignedUserId !== booking.assignedUserId.toLowerCase()
      ) {
    ...
    const bookables = await Bookable.find({
      ownerUserId: { $exists: true, $ne: "" },
    });

    for (const bookable of bookables) {
      if (
        bookable.ownerUserId &&
        bookable.ownerUserId !== bookable.ownerUserId.toLowerCase()
      ) {
    ...
    for (const coupon of coupons) {
      if (
        coupon.ownerUserId &&
        coupon.ownerUserId !== coupon.ownerUserId.toLowerCase()
      ) {
    ...
    for (const event of events) {
      if (
        event.ownerUserId &&
        event.ownerUserId !== event.ownerUserId.toLowerCase()
      ) {
    ...
    for (const groupBooking of groupBookings) {
      if (
        groupBooking.assignedUserId &&
        groupBooking.assignedUserId !==
          groupBooking.assignedUserId.toLowerCase()
      ) {
    ...
    for (const instance of instances) {
      if (instance.ownerUserIds && instance.ownerUserIds.length > 0) {
        ...
        if (
          JSON.stringify(lowercaseOwnerUserIds) !==
          JSON.stringify(instance.ownerUserIds)
        ) {
    ...
    for (const role of roles) {
      if (
        role.assignedUserId &&
        role.assignedUserId !== role.assignedUserId.toLowerCase()
      ) {
    ...
    for (const tenant of tenants) {
      ...
      if (tenant.ownerUserIds && tenant.ownerUserIds.length > 0) {
        ...
        if (
          JSON.stringify(lowercaseOwnerUserIds) !==
          JSON.stringify(tenant.ownerUserIds)
        ) {
          await Tenant.updateOne(
            { _id: tenant._id },
            { $set: { ownerUserIds: lowercaseOwnerUserIds } },
  ...
  down: async function (mongoose) {
    console.log(
      "This migration cannot be reverted as the original case of the IDs is not stored",
    );
  },
};
