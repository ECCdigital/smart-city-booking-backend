...
| accessPointDetails           | Access points the bookable references: `active`, `accessBuffer` (`before`/`after`, minutes), `accessPointIds`, and `accessPointAmounts` (`{ "<accessPointId>": <n> }`) — the compartments a booking gets at each locker system, `amount` distributed over them.    |
| lockerDetails                | Read-only. Derived from the locker rows (`type: locker`) among `accessPointDetails.accessPointIds`: `active`, and one unit per row with the compartments distributed to it; never stored, a value sent in is ignored. Locker systems are access points.            |
...
Providers are `nuki`, `salto-ks`, `ifbs` and `pareva`. `scanCode` and `previousScanCodes` are stored but never returned. Capacity is the bookable's `amount`, never the locker system's. How many compartments a booking gets at each of a bookable's locker systems is `accessPointDetails.accessPointAmounts` (`{ "<accessPointId>": <n> }`): the bookable's `amount` is distributed over its locker systems by the editor, not owed at each of them again. A locker system the map says nothing about gives what the booking's item books, as before there was one, and an amount for an access point the bookable does not reference is dropped when the bookable is saved.
