...
- Booking lifecycle, step 3 of 9: booking documents are issued by one module, `documents/document-issuance.js` (`issue({ tenantId, bookingIds, type, groupBookingId? })`, `remove`): it draws the number, renders, stores the bytes and attaches the document to every booking with one atomic `$push`, so a document number exists exactly when its attachment does, and a whole-document write can no longer lose an attachment to a state write running next to it. A second issue of a type at a booking is a revision under the same number (`revision` + 1), a reprint included: the reprint endpoints no longer hand out a fresh number. `issue` never mails. The receipt, invoice and cancellation services are renderers only; `BookingService.createReceipt`, `createAggregatedReceipt`, `createAggregatedInvoice` and `InvoiceService.issueAggregatedInvoice` are gone, the reprint endpoints and `InvoicePaymentService` call `issue`. Every document attachment now carries both `name` and `title`
