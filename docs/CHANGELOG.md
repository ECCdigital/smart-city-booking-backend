...
- Booking lifecycle, step 2 of 9: **schema change** - a booking carries its state as `booking.status` (`requested | payment_due | confirmed | rejected | cancelled`), the one source of truth; `isCommitted`, `isPayed` and `isRejected` stay in schema, database and HTTP answers as columns the `Booking` entity derives from the state on every write (`isPayed` reads as "nothing left to pay": a free booking carries it in every state, a priced one once confirmed, as the checkout set it). Assigning a flag is refused: it throws in a test run (`NODE_ENV=test`, set by `.mocharc.yml`) and is logged and ignored in production. `isBookingValid()` and the status key of `GET /bookings/:id/status` read the state; a `status` sent in a request body is ignored, the flags stay the HTTP form. Migration `03-09-2026-add-booking-status` reads the state off the flags of every booking (idempotent, `down` removes it) and records `cancellationRefund.cancelledFrom`, the state a cancelled booking was cancelled from (`confirmed` where paid or free, else `payment_due`), which the entity reads `isPayed` from and `reinstate` will return to; a cancelled booking without a refund audit gets one with just that field. New modules `booking-lifecycle/booking-state.js` (states, transition table `nextState`, flag derivation) and `booking-lifecycle/update-plan.js` (`planUpdate`: the admin PUT as a list of transitions, 400 `invalid_status_change` for flags without a way); neither is wired in yet
- Behaviour change, deliberate: "paid but never confirmed", a combination of the flags the state model does not have, now reads as `confirmed` (the payment is the stronger statement) wherever it comes in - a manual booking sent with `isPayed` only, the admin PUT sending that combination, a payment webhook for a request that was never confirmed - so such a booking is granted, gets its receipt and the confirmation mail; the migration counts existing ones in its log. Tickets 4, 7 and 9 turn these inputs into 409/400 answers
