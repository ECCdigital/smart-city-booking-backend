...
- Media CLI (`node src/cli/media-cli.js`) with `import`, `regenerate`, `verify`, `cleanup`, `purge-imported` and `purge-legacy`, each with `--dry-run` and a report of what was processed, skipped, left unplaced or failed. Deliberately not a boot migration: an update never waits on a file move. Every command is idempotent — a second run changes nothing
