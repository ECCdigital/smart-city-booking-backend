...
- The open path reads a door once: the resolver hands the stored door along with its rules and scan code, and the evidence step judges by that instead of reading the access point a second time. Internal only, HTTP shapes are unchanged
