...
- Locker fold, step 4 of 4: **schema change** - locker systems are rows of `accesspoints` (`type: locker`; an iFBS location with `externalId` = `providerLocationId` = the location, `mode: remote`; a Pareva size with `externalId` the product id, `providerLocationId` the application's `lockerId`, `mode: authorization`), referenced by the bookables like doors, and iFBS/Pareva are tenant applications of type `access`. Migration `02-09-2026-fold-lockers-into-access-points` makes the rows from every bookable's `lockerDetails.units` and every booking's `lockerInfo`, references them from `accessPointDetails.accessPointIds` (switching `accessPointDetails.active` on), turns each `lockerInfo` record into an `accessInfo` compartment (confirmed → grant, unconfirmed → hold), moves the compartments made since step 3 from their synthesized ids to the rows, keeps the iFBS price provider at its location, lowers a bookable's `amount` to its Pareva unit's where that was the smaller one (logged), retypes the applications and removes `lockerDetails` and `lockerInfo` from the documents. Idempotent; `down` puts units (with the bookable's amount), `lockerInfo` and the application type back and removes the locker rows
- `bookable.lockerDetails` is a read field derived from the referenced locker rows (`active`, one unit per row with the bookable's `amount`), attached by the bookable routes; a value sent in is ignored and nothing is stored. `externalProviders`' iFBS entry follows the referenced iFBS locker system at write time, no longer `lockerDetails` at read time
- `/locker/:provider/locations`, `…/locations/:id`, `…/locations/:id/status`, `…/locations/:id/price` and `POST …/test` stay one release as a facade for the admin UI, now over `AccessInfoService`: the locations and sizes are the provider's listed access points (the provider's own record under `metadata`, the status the same answer), the price is the checkout price provider's categories, the test is the access test registry's and needs only the `MANAGE_BOOKABLES` read permission (it wrongly demanded `MANAGE_TENANTS` on top). `GET …/customer-service-info` (unauthenticated, unused) is gone. Documented in OpenAPI as deprecated
- Both locker adapters read the tenant's application as `access` only; the fallback to `locker` is gone. `AccessService` no longer synthesizes locker rows from `lockerDetails`; capacity is always the bookable's `amount`
- Deleted: `LockerInfoService`, the locker client registry and test registry, `LockerController`, `lockerApplication.js` (iFBS and Pareva applications live in `accessApplication.js`); the iFBS and Pareva clients are registered at the access client registry
- OpenAPI: `type: locker` of an access point is in use and described; the `/locker` facade is documented for the first time
- Follow-up outside this change (admin UI): manage locker systems in the access point area, configure iFBS/Pareva as `access` applications, the bookable editor on `accessPointIds`, BookingDetails on `accessInfo`; then the `/locker` facade goes
