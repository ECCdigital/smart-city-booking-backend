...
- Access provider contract test (`tests/access-provider-contract.test.js`) run against NUKI, Salto KS and iFBS over fake API clients that replace only the HTTP transport, plus an in-memory access provider for tests. Characterization tests pin the three provider dialects `AccessService` reads today (open answers, status sniffing, `accessInfo` fields, revoke handling) and the states NUKI `getSmartlockState` reports. The adapters accept an injected client (`new NukiAccessProvider({ client })`) for that; production behaviour is unchanged
