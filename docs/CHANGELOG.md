...
- Booking lifecycle, step 8 of 9: a group booking runs its transitions through the group lifecycle, `booking-lifecycle/group-booking-lifecycle.js` (`admit | confirm | pay | cancel(tenantId, groupBookingId, options)`), over the same pipeline and the same six adapters as a single booking: the state write and the access per member (every effect row of a member carries its `bookingId`), then one aggregated document and one mail for the group, the workflow event per member, the payment request with the group. A group has no state of its own (glossary "Gruppenzustand"): every transition demands its members in one shared state and refuses others with 409 `invalid_transition` naming the deviating `bookingIds`. `POST /group-bookings/:id/commit`, `/pay` and `/reject` call the lifecycle as the administration; the consistency checks of the commit and the rejection keep their `{ success: false, errors }` answer in front of it (`BookingService.groupTransitionErrors`). `BookingService.setAggregatedBookingPayed` delegates for the payment webhook (`groupBookingId` looked up by the first member where the webhook does not name it); `commitGroupBooking`, `rejectGroupBooking` and `handleAggregatedBookingConfirmation` are gone. The group checkout hands the stored group to `admit` for the receipt, the one mail and the notices
- Behaviour change, deliberate (spec part 2, 4.2, 4.3 and 13): a state write that fails at member k of a group restores the members written before it and answers 500 with nothing else run (before, members 1..k-1 stayed in the new state, the rest did not). Confirming, paying or cancelling a group that is not in the state the transition needs - a confirmed group again, a paid group again, a cancelled group again, a group whose members differ in state - is refused with 409 `invalid_transition` at the three endpoints and the aggregated payment webhook (before, a second commit confirmed as free, a second payment or cancellation issued a second document); an unknown group is a 404. The aggregated payment fires `onPay` per member (it never did) and its confirmation carries the receipt **and** the members' `mailAttach` documents (the receipt replaced them). The group's cancellation document is issued after the state writes, so a document that fails leaves the group cancelled without it and a reprint issues it; a receipt, a document or a mail that fails is recorded and answers 200 (each was a 500 before). The organizer of a ticket member is now told on confirmation (the block read the type off the item and, for a priced group, sat after the return). The workflow events of a group fire after all its writes; the grants of a free group follow all its writes
