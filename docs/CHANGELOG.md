...
- Locker fold, step 2 of 4: compartments are real `accessInfo` entries (`accessPointType: locker`) at stored access points of type `locker`, one per compartment and told apart by `grant.authorizationId`, each carrying `bookableId`, `hold`, `compartment` and `externalBookingId`. New `AccessService.holdForBooking` (one entry per `bookableItem.amount`, `provider.hold` where the provider holds, otherwise a platform hold checked against `bookable.amount` after the store: 409 `compartments_unavailable`) and `refreshHolds`; `provisionForBooking` consumes the hold and always grants a compartment, iFBS' `mode: remote` included; `revokeForBooking` revokes every granted compartment by its own grant; `updateForBooking` revokes and re-grants all compartments when the time or the allocation (systems or amount) changed. The old synthesis of locker access points from `booking.lockerInfo` is gone; the checkout still runs through `LockerService` until step 3, so `holdForBooking`/`refreshHolds` have no caller yet
- Compartments are listed and operated under an opaque id of their own (`<accessPointId>:<authorizationId>`, `<accessPointId>:hold` before the grant) instead of the box number or process id; `GET /access` additionally carries `compartment` (iFBS' box number, `null` at Pareva), and `isProvisioned` at a compartment means granted and not revoked rather than existing. The access decision treats a compartment like a door that only takes a code - without a grant `not_provisioned`, revoked `authorization_revoked`, neither operable - and asks a compartment for the evidence its locker system's `validationRules` demand (`[]` demands nothing); `?includeLockers=true` lists compartment ids. iFBS grants name the box as `compartment`; the audit log gains the action `hold`
