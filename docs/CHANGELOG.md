...
- **Schema change** `booking.accessInfo[]`: the grant of an access point is stored as `grant: { authorizationId, externalPrincipalId, secret }` (secret encrypted) with `principalRemovedAt`, `principalCleanupAttemptedAt` and `principalCleanupError`; the flat `authorizationId`, `accessId`, `saltoUserId`, `pin`, `providerResponse` and the `saltoUser*` cleanup fields are gone. Migration `02-09-2026-move-access-grants` converts existing entries (idempotent, with `down`; `providerResponse` is not restored - the audit log holds a copy). `accessInfo` still goes out raw in the booking GET. The access provider seam answers `grantAuthorization` as a `Grant` and `revokeAuthorization(accessPoint, grant)` as a `Revocation`; a revoke is idempotent at every provider (a grant or Salto guest already gone is nothing to do, and the entry is recorded as revoked instead of left provisioned). Audit payloads of provision/revoke carry the grant without its secret. NUKI keypad grants now send the keypad type and code as the Web API numbers them and read the authorization id from the listing, since Nuki creates authorizations asynchronously and answers without one; generated NUKI codes follow the keypad rules (digits 1-9, not starting with 12)
- The Salto-only cleanup job is now the provider-neutral grant cleanup (`grant-cleanup-service.js`): it revokes again through the provider seam wherever a grant's external principal is still there after the revoke. Env `GRANT_CLEANUP_ENABLED` / `GRANT_CLEANUP_INTERVAL_MS` replace `SALTO_KS_CLEANUP_*`, which still count where they are the only ones set
