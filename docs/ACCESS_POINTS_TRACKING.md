...
Frontend-Implementierungs-Guide (Tenant-/Bookable-/Booking-Konfiguration):
[ACCESS_POINTS_FRONTEND.md](./ACCESS_POINTS_FRONTEND.md).
...
| **B. Salto-KS-Integration**       | 🟡     | B1-B4 fertig · 4 / 5 Phasen                                             |
...
| #    | Aufgabe                                                               | Plan-Ref            | Status | Notiz                                                                                                                                                                           |
| ---- | --------------------------------------------------------------------- | ------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B4.1 | Salto-User-Lifecycle pro Buchung (`saltoUserId`/`accessId` speichern) | Phase 6.3           | ✅     | `AccessService` persistiert `accessId`/`saltoUserId`, reicht sie beim Revoke weiter und markiert `saltoUserDeletedAt`/Cleanup-Fehler                                            |
| B4.2 | Cleanup-Job für orphaned Salto-User                                   | Phase 6.3 / 7-Offen | ✅     | `salto-ks-cleanup-service` scannt widerrufene Salto-AccessInfos und löscht Users periodisch; `SALTO_KS_CLEANUP_INTERVAL_MS`, deaktivierbar via `SALTO_KS_CLEANUP_ENABLED=false` |
| B4.3 | Webhook-Route `POST /api/webhooks/access/salto-ks/:tenant`            | Phase 7b.1          | ✅     | generische Route `POST /api/webhooks/access/:provider/:tenant` deckt `salto-ks` ab                                                                                              |
| B4.4 | Auto-Register Webhook bei Aktivierung der Salto-App                   | Phase 7b.3          | ✅     | `access-app-lifecycle-service` registriert/unregistriert Salto-Webhooks beim Tenant-Update; Callback über `ACCESS_WEBHOOK_BASE_URL` oder `BACKEND_URL`                          |
